...
function pageVars(dict) {
  ...
  return {
    ...
    d: '—',
    t: '—',
    // 「已点亮」是访客自己的本地记录，静态页不可能替某个匿名访客算出来——
    // 用破折号占位，client 上 renderLitPill() 会立刻用真实值覆盖它。
    // 注意不能复用上面的 n：那个已经被 barsNote/privacyWeb 的"地图共 152 城"
    // 占用，硬凑同一个变量会连带把 heroLive 的实时统计也顶成假的静态数字。
    lit: '—',
  };
}
...
export function buildPages() {
  ...
  // 执行是常态，所以非英语访客总要先看一屏英文。两段内联脚本把首屏那一小撮键
