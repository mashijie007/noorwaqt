...
| `build-og.mjs` | 153 张链接预览图（1200×630），Playwright 驱动 `tools/og/` 渲染台 |
...
分享一条城市页链接时，聊天软件抓 `og:image` 展开成预览卡。
在这之前 1591 个页面共用同一张方形 logo —— 发雅加达的链接和发喀拉蚩的，
群里看到的预览一模一样。

现在一城一张，外加一张站点级的 `_site.jpg`（44 个语言首页与无城市页语言的兜底）。

**图上不写礼拜时刻**，这是最重要的一个决定。聊天软件会把 `og:image` 缓存很久，
一条几周前分享的链接，预览里仍是当时抓到的那张图；礼拜时刻每天挪一两分钟，
一个月能差出小半个钟头 —— 一个礼拜应用的预览里挂着错时刻，比不写糟得多。
所以图上只放不会过期的：城市名、国家、以它为中心的地球、朝向角、到麦加的距离。
**今日时刻走 `og:description`** —— 纯文本，抓取端每次重抓都会更新。

地球按**当地太阳正午**渲染，不按此刻：按此刻会有一半城市落在夜面上，整张图黑成一片。

用 Playwright 而不是 node-canvas：globe.js 依赖 ResizeObserver、devicePixelRatio、
getBoundingClientRect，在 Node 里得逐个打桩，而打歪不会报错，只会让 153 张图悄悄画错。
本地 153 张约 8 秒。没装 playwright 时这一步自动跳过，`og:image` 退回站点 logo。
