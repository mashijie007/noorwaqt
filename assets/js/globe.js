...
export class Globe {
  ...
  draw() {
    ...
    for (let i = 0; i < LAND_COUNT; i++) {
      ...
      // 那一带本来也几乎没有人烟，索性不画，把视线让给有人的纬度带。
      //
      // 判据必须是 y，不能是 yt。y 就是 sin(地理纬度)，切出来是一顶固定的地理极冠
      // （北纬 64.16° 以北 / 南纬 60.46° 以南）。yt 是倾斜 23.44° 之后的屏幕竖向，
      // 算式 -x1*st + y*ct 里混进了经度，帽子会跟着地球一起转 —— 那样正面可见的
      // 陆地点会被剔掉 8%~24%，数量随转角在 389~745 之间摆动，最深吃到北纬 41°，
      // 同一块陆地转过去再转回来会一亮一灭。下面的 y2 / lit 仍用 yt，那是投影和昼夜，
      // 本来就该在倾斜后的坐标里算。
