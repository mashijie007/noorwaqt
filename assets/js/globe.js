...
export const THEME = {
   ...
   *
   * 这四个数是有代价换来的。陆地是 7701 个格点，纬向步进 1.25°：在 720px 的球上
   * 点距约 8.1px，而点本身只有 2px。稀疏 + 高对比 + 规则网格，看久了是会让人
   * 生理不适的那种构型（密集恐惧）。一度为了强调晨昏线把亮端推到 rgb(205,255,225)，
   * 对比拉到 12.27×，结果不是让分界更清楚，是把网格本身放大了。
   *
   * 现在的做法反过来：晨昏线要的是「昼夜分明」，而那来自比值不是绝对亮度。
   * 夜侧那一档压到与球体底色只差 1.3 的亮度、彻底融进去 —— 分界因此是
   * 「有陆地 / 没陆地」，结构上比拼亮度更强；白昼一侧只给到 108（曾经是 242），
   * 网格也就不再跳出来抢眼睛。
   *
  ...
  city: {
    ...
    size: [0.042, 0.054],     // R 的倍数：底 + 随亮度增加的部分
    alpha: [0.36, 0.6],
    core: [1.05, 1.45],       // 中心实心点的半径（像素）
